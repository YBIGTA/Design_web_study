...
-  Toggle :"왔다갔다 하다"라는 의미로서, hide와 slow 를 **번갈아가며 실행**하는 메소드
...
- $(selector).show(speed, callback)[toggle 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_toggle)
...
### 문법
 ...
 숨겨진 인자들을 보이게 함.[fadeIn 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_fadein)
   
	$(selector).fadeOut(speed, callback) 

보이는 인자들을 숨김. [fadeOut 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_fadeout)

	$(selector).fadeToggle(speed, callback) 
인자들이 숨겨졌을 경우 fadeIn을 , 
인자들이 화면에 나타나있을 경우 fadeOut을 실행함. [fadeToggle 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_fadetoggle)
...
### 개요
	...
	- 인자(element)들을 올리거나(slide up) 내리는(slide down) 메소드
	- 
  
### 문법
- $(selector).slideDown(speed, callback) : 인자를 끌어올림[slideDown 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_slide_down)
- $(selector).slideUp(speed, callback) : 인자를 끌어내림[slideUP 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_slide_up)
- $(selector).slideToggle(speed, callback) : 
	인자가 slideUp되있을 경우 slideDown을, slideDown 되있을 경우 slideUp을 실행[slideToggle 예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_slide_toggle)
...
- {params } : required parameter로서, 애니메이션화 될 CSS특성을 지정.
- speed : effect를 실행하는데 소요되는 시간(duration)을 지정.
- callback : 애니메이션이 실행완료된 이후에 실행된 메소드 지정. [예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_animation1)

------

## 4-2. Animate() - 여러 특성들 지정하기

- animate()를 이용하여 **동시에** 다양한 특성들을 지정할 수 있음. [예시](https://www.w3schools.com/jquery/tryit.asp?filename=tryjquery_animation1_multicss)
...
- "show", "hide", "toggle" 과 같은 특성의 animation value를 할당 가능.
...
- jQuery stop() **에니메이션**이 끝나기 전에 에니메이션을 멈추는 메소드
...
- callback 파라미터는 hide를 먼저 실행시킨 후 , hide effect가 **끝난 후 ** callback에 해당하는 메소드를 실행시키도록 지정.
...
### 2. 문법(예시)
> $(selector).method1().method2(2000).method3(2000);
 
 - selector를 통해 특정 인자를 불러오고 특정 인자에 대해 여러 메소드들을 적용시켜 주는 주는 방식.

 - **Tip** : Chaining을 하다가 문장이 길어질 경우 줄을 바꾸거나 들여쓰기를 해도 됨.(jQuery문법은 그렇게 엄격하지 않음)
